// RSA Key Generation, Storage, and Use
...
// AES encryption for image
export async function encryptImageData(publicKey, base64Image) {
  const aesKey = await crypto.subtle.generateKey({ name: "AES-GCM", length: 256 }, true, ["encrypt", "decrypt"]);
  const iv = crypto.getRandomValues(new Uint8Array(12));
  const encryptedImage = await crypto.subtle.encrypt({ name: "AES-GCM", iv }, aesKey, new TextEncoder().encode(base64Image));
  const exportedAesKey = await crypto.subtle.exportKey("raw", aesKey);
  const encryptedAesKey = await crypto.subtle.encrypt({ name: "RSA-OAEP" }, publicKey, exportedAesKey);
  return {
    encryptedImage: btoa(String.fromCharCode(...new Uint8Array(encryptedImage))),
...
export async function decryptImageData(privateKey, encryptedImage, encryptedKey, iv) {
  const aesKeyRaw = await crypto.subtle.decrypt({ name: "RSA-OAEP" }, privateKey, Uint8Array.from(atob(encryptedKey), c => c.charCodeAt(0)));
